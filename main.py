...
async def info(mess: a.types.Message):
    a = '🔸<b>Адрес кампуса на Большой Семёновской:</b>\n' \
        ...
        'Проезд:\n' \
        '- от станции метро «Автозаводская» (выход №2) на автобусах 44, 142, т40 до ост. «Парк Легенд», далее пешком 2 минуты;\n' \
        '- от станции метро «Автозаводская» (выход №3) на автобусах 9, 99, 186, Т26, Т67 до ост. «Парк Легенд», далее пешком 2 минуты.\n' \
        '🔸<b>Адрес учебного корпуса на ст. м. «ВДНХ»:</b>\n' \
        'ул. Павла Корчагина, д. 22.\n' \
        'Проезд:\n' \
        '- от станции метро «ВДНХ» (выход №4 и №5) на автобусах 378, 286 до ост. «Гимназия», далее пешком меньше 5 минут;\n' \
        '- от станции метро «ВДНХ» (выход №1) на трамваях 11, 25 до ост. «Пл. Академика Люльки», далее пешком 8 минут;\n' \
        '- от станции метро «Алексеевская» пройти до остановки на ул. Мытищинская 3-я пешком 6 минут, сесть на автобус 714 до ост. «Гимназия», далее пешком 3 минуты.\n' \
        '🔸<b>Адрес учебного корпуса на ул. Прянишникова:</b>\n' \
        '127550, г. Москва, ул. Прянишникова, 2А\n' \
        'Корпуса 1, 2\n' \
        'Проезд:\n' \
        '- от станции метро «Петровско-Разумовская» (первая линия остановок от метро), платформы «Петровско-Разумовская» на автобусах 114, 204, 282, т19, 300 до ост. «Кинотеатр «Байкал»» (15–20 минут), далее пешком 8 минут вдоль пруда;\n' \
        '- от станции метро «Войковская» на автобусах 204, 179, 282, 591, 114 до ост. «Михалковская улица» (15–20 минут), далее пешком 8 минут вдоль пруда;\n' \
        '- от станции метро «Дмитровская» на трамвае 29, 27 или «Войковская» на трамвае 27 до ост. «Политехнический университет» (15–20 минут);\n' \
        '- от Савёловского вокзала или станции метро «Савёловская» на автобусах 72, 87 до ост. «Политехнический университет»;\n' \
        '- от станции МЦК «Коптево» на трамвае 27к, автобусах 22, 595, 72, 801, 87 до ост. «Политехнический университет» или пешком 20 минут;\n' \
        '- от платформы «Красный Балтиец» на автобусах т57, т19, 300 до ост. «Михалковская улица», далее пройти назад и идти пешком вдоль пруда 7 минут;\n' \
        '- от станции метро «Селигерская» на автобусах 591, 179 до ост. «Михалковская улица» (15–20 минут), далее пешком 8 минут вдоль пруда.\n'
    await mess.bot.delete_message(mess.from_user.id, mess.message_id)
...
async def new_idea(mess: a.types.Message):
    await mess.bot.delete_message(mess.from_user.id, mess.message_id)
    ideas = BotDB.cursor.execute("SELECT * FROM `ideas` WHERE `user_id` = ? AND `status` = ?", (mess.from_user.id, 'wait')).fetchall()
    if len(ideas) == 0:
...
async def new_idea_yes(call: a.types.CallbackQuery):
    ...
    BotDB.conn.commit()
    await call.bot.edit_message_reply_markup(chat_id=call.from_user.id, message_id=call.message.message_id, reply_markup=None)
